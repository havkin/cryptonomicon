    ...
    <div class="container">
      ...
      <template v-if="tickers.length">
        ...
        <div>
          ...
          <input
            v-model="filter"
            class="w-1/5 pr-10 border-gray-300 text-gray-900 focus:outline-none focus:ring-gray-500 focus:border-gray-500 sm:text-sm rounded-md"
            type="text"
          />
          <div class="mt-4">
            <button
              v-if="page > 1"
              @click="page--"
              class="mr-3 inline-flex items-center py-2 px-4 border border-transparent shadow-sm text-sm leading-4 font-medium rounded-full text-white bg-gray-600 hover:bg-gray-700 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500"
            >
              Назад
            </button>
            <button
...
export default {
  ...
  methods: {
    subscribeToUpdates(tickerName) {
      const id = setInterval(async () => {
        const res = await fetch(
          `https://min-api.cryptocompare.com/data/price?fsym=${tickerName}&tsyms=USD&api_key=05c7ec6da4333710c1a42ef8f9550db1c974b203285cd9777465150046ad47df`
        );
        const data = await res.json();
        if ("USD" in data) {
      ...
      this.tickers.find((t) => t.name === tickerName).intervalId = id;
    },
    ...
    remove(item) {
      ...
      this.tickers = this.tickers.filter((ticker) => ticker != item);
  ...
  watch: {
    ...
    tickersToShow() {
      if (this.tickersToShow.length === 0 && this.page > 1) {
        this.page -= 1;
      }
    },
